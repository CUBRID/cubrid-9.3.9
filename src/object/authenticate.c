...
#define IS_ENCODED_ANY(string) \
        (IS_ENCODED_DES(string) || IS_ENCODED_SHA1(string) || IS_ENCODED_SHA2_512(string))
...
encrypt_password_sha2_512 (const char *pass, char *dest)
{
  ...
  if (pass == NULL)
    ...
    {
      error_status = sha_two (NULL, pass, strlen (pass), 512,
			      &result_strp, &result_len, true);
...
{
  return (au_set_password_internal (user, password, 1,
				    ENCODE_PREFIX_DEFAULT));
}
				     ...
				     DB_VALUE * password)
{
  ...
  if (password != NULL)
      ...
      if (IS_STRING (password))
	{
      ...
      /* in case of SHA2, prefix was not stripped off */
      if (string != NULL && IS_ENCODED_SHA2_512 (string))
	{
	  error = au_set_password_internal (user, string + 1, 0,
					    ENCODE_PREFIX_SHA2_512);
	}
      else
	{
	  error = au_set_password_internal (user, string, 0,
					    ENCODE_PREFIX_SHA1);
	}
