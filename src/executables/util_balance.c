...
#ifdef UTIL_BROKER_NAME
#undef UTIL_BROKER_NAME
#endif

#define UTIL_BROKER_NAME "cubrid_broker_bal" UTIL_EXE_EXT
...
static int proc_execute_hide_args (const char *file, const char *args[],
				   bool wait_child, bool close_output,
				   bool close_err, int *out_pid);
static int proc_execute_internal (const char *file, const char *args[],
				  bool wait_child, bool close_output,
				  bool close_err, bool hide_args,
				  int *out_pid);
...
process_admin (int argc, char **argv)
{
  ...
  status = proc_execute_hide_args (UTIL_ADMIN_NAME, (const char **) copy_argv,
...
{
  return proc_execute_internal (file, args, wait_child, close_output,
				close_err, false, out_pid);
}

static int
proc_execute_hide_args (const char *file, const char *args[], bool wait_child,
			bool close_output, bool close_err, int *out_pid)
{
  return proc_execute_internal (file, args, wait_child, close_output,
				close_err, true, out_pid);
}

#if defined(WINDOWS)
static int
proc_execute_internal (const char *file, const char *args[], bool wait_child,
		       bool close_output, bool close_err, bool hide_args,
		       int *out_pid)
{
  STARTUPINFO si;
