...
bool is_select_query(const char *sql_stmt)
{
  int i;

  for (i = 0; i < strlen(sql_stmt); i++) {
    if (isspace(sql_stmt[i])) continue;
    if (!strncasecmp(sql_stmt, "select", 6)) return true;
	else break;
  }
...
sp_create_parser (const char *sql_stmt)
{
  ...
  parser_p->is_select = is_select_query(sql_stmt);
...
sp_get_hint_type (const char *sql, SP_HINT_TYPE * hint_type)
{
  ...
  *hint_type = HT_NONE;
  if (strncasecmp (sql, "load_balance", 12) == 0)
    {
      sql += 12;
      *hint_type = HT_BAL;
      return sql;
