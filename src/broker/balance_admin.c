...
static void
ha_config_read_and_init_connection(char *connection_file)
{
  FILE *fp;
  ...
  char *start_node;
  char **ha_node_list_pp;
  char **ha_db_list_pp;
  char path[BROKER_PATH_MAX];

  sysprm_load_and_init(NULL, NULL);

  envvar_confdir_file (path, BROKER_PATH_MAX, connection_file);

  fp = fopen(path, "w");
  if (!fp)
    {
      printf("ha_config_read_and_init_connection: cannot open file [%s]\n", path);
      return;
    }
  ...
  if (ha_db_list_p)
    {
      ha_db_list_pp = util_split_string (ha_db_list_p, " ,:");
    }

  if (ha_node_list_p)
    {
      num = util_get_num_of_ha_nodes (ha_node_list_p);
      start_node = strchr (ha_node_list_p, '@');
      ha_node_list_pp = util_split_string (start_node + 1, " ,:");

	  for (i = 0; i < num; i++)
	    {
          fprintf(fp, "%d\t%s\t%s\n", i, ha_db_list_pp[0], ha_node_list_pp[i]);
	    }
    }

  if (ha_replica_list_p)
    {
      num = util_get_num_of_ha_nodes (ha_replica_list_p);
      start_node = strchr (ha_node_list_p, '@');
      ha_node_list_pp = util_split_string (start_node + 1, " ,:");
...
main (int argc, char **argv)
{
  ...
  ha_config_read_and_init_connection(br_info->shard_connection_file);
