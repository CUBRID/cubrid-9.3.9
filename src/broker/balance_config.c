...
/*
 * balance_config.c - broker configuration utilities
 */
...
#define DEFAULT_BALANCE_PROXY_LOG_MODE		"ERROR"
#define DEFAULT_BALANCE_KEY_MODULAR	        256
#define DEFAULT_BALANCE_PROXY_TIMEOUT 		"30s"
#define DEFAULT_BALANCE_PROXY_CONN_WAIT_TIMEOUT   "8h"
...
static T_CONF_TABLE tbl_proxy_log_mode[] = {
  ...
  {"BALANCE", PROXY_LOG_MODE_BALANCE_DETAIL},
...
#define PRINTERROR(...)	\
  ...
  for (i = 0; i < ini->nsec; i++)
      ...
      else
	{
      ...
      br_info[num_brs].shard_flag = conf_get_value_table_on_off (ini_getstr (ini, sec_name, "BALANCE", "OFF", &lineno));
      if (br_info[num_brs].shard_flag < 0)
	{
      ...
      /* BALANCE PHASE0 */
      br_info[num_brs].proxy_shm_id = ini_gethex (ini, sec_name, "BALANCE_PROXY_SHM_ID", 0, &lineno);

      strncpy (br_info[num_brs].shard_db_name,
	       //ini_getstr (ini, sec_name, "BALANCE_DB_NAME", DEFAULT_EMPTY_STRING, &lineno),
		   "ha_db",
	       sizeof (br_info[num_brs].shard_db_name));

#if 0
      strncpy (br_info[num_brs].shard_db_user,
	       ini_getstr (ini, sec_name, "BALANCE_DB_USER", DEFAULT_EMPTY_STRING, &lineno),
	       sizeof (br_info[num_brs].shard_db_user));

      strncpy (br_info[num_brs].shard_db_password,
	       ini_getstr (ini, sec_name, "BALANCE_DB_PASSWORD", DEFAULT_EMPTY_STRING, &lineno),
	       sizeof (br_info[num_brs].shard_db_password));
#endif

      br_info[num_brs].num_proxy = ini_getuint (ini, sec_name, "BALANCE_NUM_PROXY", DEFAULT_BALANCE_NUM_PROXY, &lineno);
      ...
      if (br_info[num_brs].num_proxy > MAX_PROXY_NUM)
	{
      ...
      strcpy (br_info[num_brs].proxy_log_dir,
	      ini_getstr (ini, sec_name, "BALANCE_PROXY_LOG_DIR", DEFAULT_SHARD_PROXY_LOG_DIR, &lineno));

      br_info[num_brs].proxy_log_mode =
	conf_get_value_proxy_log_mode (ini_getstr
				       (ini, sec_name, "BALANCE_PROXY_LOG", DEFAULT_BALANCE_PROXY_LOG_MODE, &lineno));
	...
	ini_getuint (ini, sec_name, "BALANCE_MAX_CLIENTS", DEFAULT_BALANCE_MAX_CLIENTS, &lineno);
      if (br_info[num_brs].max_client > CLIENT_INFO_SIZE_LIMIT * br_info[num_brs].num_proxy)
	{
      ...
      strncpy (br_info[num_brs].shard_connection_file,
	       //ini_getstr (ini, sec_name, "BALANCE_CONNECTION_FILE", "balance_connection.txt", &lineno),
	       "balance_connection.txt",
      ...
      if (br_info[num_brs].shard_connection_file[0] == '\0')
	{
      ...
      strncpy (br_info[num_brs].shard_key_file, ini_getstr (ini, sec_name, "BALANCE_KEY_FILE", "shard_key.txt", &lineno),
	...
	ini_getuint (ini, sec_name, "BALANCE_KEY_MODULAR", DEFAULT_BALANCE_KEY_MODULAR, &lineno);
      if (br_info[num_brs].shard_key_modular < 0)
	{
	  errcode = PARAM_BAD_VALUE;
	  goto conf_error;
	}
      strcpy (library_name, ini_getstr (ini, sec_name, "BALANCE_KEY_LIBRARY_NAME", DEFAULT_EMPTY_STRING, &lineno));
      ...
      else
	{
      ...
      strcpy (br_info[num_brs].shard_key_function_name,
	      ini_getstr (ini, sec_name, "BALANCE_KEY_FUNCTION_NAME", DEFAULT_EMPTY_STRING, &lineno));

      strncpy (size_str,
	       ini_getstr (ini, sec_name, "BALANCE_PROXY_LOG_MAX_SIZE", DEFAULT_BALANCE_PROXY_LOG_MAX_SIZE, &lineno),
	       sizeof (size_str));
	...
	ini_getint (ini, sec_name, "BALANCE_MAX_PREPARED_STMT_COUNT", DEFAULT_PROXY_MAX_PREPARED_STMT_COUNT, &lineno);
      if (br_info[num_brs].proxy_max_prepared_stmt_count < 1)
	{
      ...
      br_info[num_brs].ignore_shard_hint = ON;
#if 0
	conf_get_value_table_on_off (ini_getstr (ini, sec_name, "BALANCE_IGNORE_HINT", "OFF", &lineno));
      if (br_info[num_brs].ignore_shard_hint < 0)
	{
      ...
      strncpy (time_str, ini_getstr (ini, sec_name, "BALANCE_PROXY_TIMEOUT", DEFAULT_BALANCE_PROXY_TIMEOUT, &lineno),
	       sizeof (time_str));
      ...
      else if (br_info[num_brs].proxy_timeout > MAX_PROXY_TIMEOUT_LIMIT)
	{
      ...
      strncpy (time_str,
	       ini_getstr (ini, sec_name, "BALANCE_PROXY_CONN_WAIT_TIMEOUT", DEFAULT_BALANCE_PROXY_CONN_WAIT_TIMEOUT,
  ...
  if (admin_flag)
      ...
      for (i = 0; i < num_brs; i++)
	{
	  ...
	  if (br_info[i].shard_flag == ON)
		...
		{
		  PRINTERROR ("config error, %s, BALANCE_PROXY_SHM_ID\n", br_info[i].name);
		  error_flag = TRUE;
		}
	      if (br_info[i].shard_db_name[0] == '\0')
		{
		  PRINTERROR ("config error, %s, BALANCE_DB_NAME\n", br_info[i].name);
		  error_flag = TRUE;
		}
	    }
	}			/* end for (i) */
...
broker_config_dump (FILE * fp, const T_BROKER_INFO * br_info, int num_broker, int br_shm_id)
{
  ...
  for (i = 0; i < num_broker; i++)
      ...
      if (br_info[i].shard_flag == OFF)
	{
      ...
      fprintf (fp, "BALANCE_DB_NAME\t\t=%s\n", br_info[i].shard_db_name);
      fprintf (fp, "BALANCE_DB_USER\t\t=%s\n", br_info[i].shard_db_user);

      fprintf (fp, "BALANCE_NUM_PROXY\t\t=%d\n", br_info[i].num_proxy);
      fprintf (fp, "BALANCE_PROXY_LOG_DIR\t\t=%s\n", br_info[i].proxy_log_dir);
      tmp_str = get_conf_string (br_info[i].proxy_log_mode, tbl_proxy_log_mode);
      if (tmp_str)
	{
	  fprintf (fp, "BALANCE_PROXY_LOG\t\t\t=%s\n", tmp_str);
	}
      fprintf (fp, "BALANCE_MAX_CLIENTS\t\t=%d\n", br_info[i].max_client);

      fprintf (fp, "BALANCE_CONNECTION_FILE\t\t=%s\n", br_info[i].shard_connection_file);
      fprintf (fp, "BALANCE_KEY_FILE\t\t=%s\n", br_info[i].shard_key_file);

      fprintf (fp, "BALANCE_KEY_MODULAR\t\t=%d\n", br_info[i].shard_key_modular);
      fprintf (fp, "BALANCE_KEY_LIBRARY_NAME\t\t=%s\n", br_info[i].shard_key_library_name);
      fprintf (fp, "BALANCE_KEY_FUNCTION_NAME\t\t=%s\n", br_info[i].shard_key_function_name);
      fprintf (fp, "BALANCE_PROXY_LOG_MAX_SIZE\t\t=%d\n", br_info[i].proxy_log_max_size);
      fprintf (fp, "BALANCE_MAX_PREPARED_STMT_COUNT\t\t=%d\n", br_info[i].proxy_max_prepared_stmt_count);
      tmp_str = get_conf_string (br_info[i].ignore_shard_hint, tbl_on_off);
      if (tmp_str)
	{
	  fprintf (fp, "BALANCE_IGNORE_HINT\t\t=%s\n", tmp_str);
	}
      fprintf (fp, "BALANCE_PROXY_TIMEOUT\t\t=%d\n", br_info[i].proxy_timeout);
      fprintf (fp, "BALANCE_PROXY_SHM_ID\t\t=%d\n", br_info[i].proxy_shm_id);
