  ...
  if (ctx_p)
    {
      ...
      PROXY_LOG (PROXY_LOG_MODE_BALANCE_DETAIL,
			       ...
			       int shard_id, int cas_id)
{
  ...
  cas_io_p =
    proxy_cas_alloc_by_ctx (ctx_p->client_id, shard_id, cas_id,
			    waiter_p->ctx_cid, waiter_p->ctx_uid,
			    ctx_p->wait_timeout, ctx_p->func_code, false);
  if (cas_io_p == NULL)
    {
