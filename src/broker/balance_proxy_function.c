			 ...
			 int argc, char **argv)
{
  ...
  if (stmt_p->parser)
    {
	  T_CLIENT_INFO *client_info_p = shard_shm_get_client_info (proxy_info_p, ctx_p->client_id);
      if (!has_shard_val_hint || client_info_p->isolation_level > TRAN_REP_CLASS_UNCOMMIT_INSTANCE)
        {
          stmt_p->parser->is_select = false;
        }
    }
...
relay_prepare_request:
  ...
  if (stmt_p)
    {
      // has_shard_val_hint means "load_balance" hint
      if (stmt_p->parser)
      	{
          is_select = stmt_p->parser->is_select;
        }
    }
  ...
  if (cas_io_p)
    {
      stmt_p->shard_id = cas_io_p->shard_id;
    }
			       ...
			       int bind_value_index)
{
  ...
  if (stmt_p) {
    ctx_p->shard_id = stmt_p->shard_id;
	if (stmt_p->parser) {
      is_select = stmt_p->parser->is_select;
    }
  }
				  ...
				  char **argv)
{
  ...
  if (ctx_p->is_in_tran)
	...
	proxy_cas_alloc_by_ctx (ctx_p->client_id, ctx_p->shard_id,
				...
				ctx_p->wait_timeout, func_code, false);
      if (cas_io_p == NULL)
	{
	  goto free_context;
	}
				     ...
				     char **argv)
{
  ...
  organized_sql_stmt = 
    shard_stmt_rewrite_sql (&has_shard_val_hint, sql_stmt, proxy_info_p->appl_server);
